...
- **Atendente/Operador** — usuário autenticado que opera o sistema em nome da cooperativa (cadastra correntistas, abre contas, realiza operações).
...
> Observação: o desafio não define perfis de cliente final self-service; assume-se que todas as operações são feitas por um operador autenticado via JWT.

## Diagrama de Casos de Uso (visão textual)

```mermaid
graph TD
    A[Atendente/Operador] --> UC1[Efetuar Login]
    A --> UC2[Cadastrar Correntista]
    A --> UC3[Consultar Correntista]
    A --> UC4[Abrir Conta]
    A --> UC5[Consultar Conta]
    A --> UC6[Realizar Depósito]
    A --> UC7[Realizar Saque]
    A --> UC8[Consultar Extrato]
    A --> UC9[Aplicar Rendimento]
    A --> UC10[Aplicar Juros]
```
...
**Fluxo principal:**
1. Operador envia credenciais (usuário/senha) para `POST /auth/login`.
2. Sistema valida credenciais.
3. Sistema gera e retorna token JWT.
**Fluxo alternativo:** Credenciais inválidas → retorna `401 Unauthorized`.

## UC02 — Cadastrar Correntista
...
## UC09 — Aplicar Rendimento (Diferencial)
...
## UC10 — Aplicar Juros (Diferencial)
